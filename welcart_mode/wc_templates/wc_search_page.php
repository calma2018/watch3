	...
	if ( have_posts() ) :
		...
		<section class="post" id="<?php echo esc_attr( $post->post_name ); ?>">
			...
			<div id="searchbox" class="search-li">
				...
				<form name="searchindetail" action="<?php echo esc_url( welcart_mode_get_cart_url() ); ?>usces_page=search_item" method="post">

					<div class="field">
						<label class="outlabel"><?php esc_html_e( 'Categories: AND Search', 'usces' ); ?></label><?php echo usces_categories_checkbox( 'return' ); ?>
					</div>

					<div class="send">
						<input name="usces_search_button" class="usces_search_button" type="submit" value="<?php esc_attr_e( 'Search', 'usces' ); ?>" onclick="newsubmit()" />
				...
				<div class="title"><?php esc_html_e( 'Search results', 'usces' ); ?>&nbsp;&nbsp;<?php echo number_format( $my_query->found_posts ); ?><?php esc_html_e( 'cases', 'usces' ); ?></div>
					...
					<?php if ( $my_query->have_posts() ) : ?>
						...
						<?php if ( ! empty( $search_result ) ) : ?>
							<?php echo wp_kses_post( $search_result ); ?>
				<?php else : ?>
					<?php
