				...
				<div id="delivery-info">
					...
					<nav class="cart-navi">
						<ul>
							...
							<li class="current"><h2><?php esc_html_e( '3.Deli. & Pay.', 'usces' ); ?></h2></li>
					...
					<div class="column1080">
						...
						<form action="<?php usces_url( 'cart' ); ?>" method="post">
							...
							<?php if ( welcart_mode_have_shipped() ) : ?>
								<table class="customer_form" id="delivery_flag">
									<tr>
										...
										<td><input name="delivery[delivery_flag]" type="radio" id="delivery_flag1" onclick="document.getElementById('delivery_table').style.display = 'none';" value="0" <?php checked( $usces_entries['delivery']['delivery_flag'], 0 ); ?> onKeyDown="if (event.keyCode == 13) {return false;}" />
										<label for="delivery_flag1"><?php esc_html_e( 'same as customer information', 'usces' ); ?></label></td>
									</tr>
									<tr>
										<td><input name="delivery[delivery_flag]" id="delivery_flag2" onclick="document.getElementById('delivery_table').style.display = 'table'" type="radio" value="1"  <?php checked( $usces_entries['delivery']['delivery_flag'], 1 ); ?> onKeyDown="if (event.keyCode == 13) {return false;}" />
										<label for="delivery_flag2"><?php esc_html_e( 'Chose another shipping address.', 'usces' ); ?></label></td>
									</tr>
								</table>
							...
							<div class="send">
								...
								<input name="backCustomer" type="submit" class="back_to_customer_button" value="<?php esc_html_e( 'Back', 'usces' ); ?>"<?php echo apply_filters( 'usces_filter_deliveryinfo_prebutton', null ); ?> />&nbsp;&nbsp;
								<input name="confirm" type="submit" class="to_confirm_button" value="<?php esc_html_e( ' Next ', 'usces' ); ?>"<?php echo apply_filters( 'usces_filter_deliveryinfo_nextbutton', null ); ?> />
							</div>
